...
def get_examples_of(word: str):
    link = "https://dic.daum.net/search.do?q={query}&dic=kor&search_first=Y" \
        .format(query=word)
    ...
    # Search result Null
    try:
        example_area = driver.find_element_by_class_name('list_example')
    ...
    # example result area
    examples = example_area.find_elements_by_class_name('txt_example')

    for example in examples:
        eg_text = example.text.split('(→')[0]
        egs.append(eg_text)
        print('.', end='')
...
def get_daumdict_examples_from(target_dictionary: str):
